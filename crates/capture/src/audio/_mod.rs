...
#[cfg(target_os = "windows")]
type PlatformLoopController = crossbeam::channel::Sender<AudioLoopCommand>;
#[cfg(target_os = "linux")]
...
impl DeviceRegistryInner {
    fn new(controller: crossbeam::channel::Sender<AudioLoopCommand>) -> Self {
        Self {
            input: vec![],
            output: vec![],
            tasks: vec![],
...
#[cfg(target_os = "windows")]
type PlatformCapture = windows::capture::WindowsCapture;

#[cfg(target_os = "linux")]
type PlatformPlayback = linux::LinuxPlayback;
#[cfg(target_os = "windows")]
type PlatformPlayback = windows::playback::WindowsPlayback;
