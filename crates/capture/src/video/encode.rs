...
use crate::video::wrapper::{
    DrmFrame, DrmInfo, Filter, Graph, HWFrameContext, HWFrameContextBuilder, Parser,
    VaapiDeviceCtx, VaapiFrame, VulkanDeviceCtx, VulkanFrame,
};
...
impl VAAPIEncoder {
    ...
    pub fn new(
        ...
        let device = VaapiDeviceCtx::new().expect("Failed to open GPU Device");
        ...
        Parser::new(&graph)
            ...
            .parse("[in]scale_vaapi=format=nv12:out_range=full[out]");
...
impl VulkanEncoder {
    pub fn new(
        ...
        let codec = encoder::find_by_name("h264_vulkan").expect("Failed to find Video Codec");
        let mut video = codec::Context::new_with_codec(codec)
            .encoder()
            .video()
            .expect("Failed to alloc codec context");
        ...
        let device = VulkanDeviceCtx::new().expect("Failed to open GPU Device");
        let hw_frame_ctx = HWFrameContextBuilder::new(&device)
            .expect("Failed to allocate memory on GPU")
            .set_format(AVPixelFormat::AV_PIX_FMT_VULKAN)
            // TODO: We should accept this as a parameter (comes from the format negotiation)
            .set_sw_format(AVPixelFormat::AV_PIX_FMT_BGR0)
            .set_width(width as i32)
            .set_height(height as i32)
        ...
        let source_filter = graph
            .create_buffer_filter("Source", |this| {
                this.set_format(AVPixelFormat::AV_PIX_FMT_VULKAN)
                    .set_hw_frame_ctx(hw_frame_ctx.clone())
                    .set_width(width as i32)
                    .set_height(height as i32)
                    .set_time_base(time_base)
        ...
        let sink_filter = graph
            .create_buffersink_filter("Sink", |this| {
                this.set_pixel_formats(&[AVPixelFormat::AV_PIX_FMT_VULKAN])
                    .expect("Failed to set pixel format")
            })
            .expect("Failed to create buffersink filter");
        ...
        Parser::new(&graph)
            ...
            .input("out", &sink_filter, 0)
            .with_gpu_device(&device)
            // Awful hack with changing widht/height but otherwise scaling end up not in Vulkan shader but
            // in libswscaler which does not support conversion from RGB to NV12 (LIKE WHAT IN THE ACTUAL FUCK)
            .parse("[in]setparams=colorspace=smpte170m:range=full,scale_vulkan=w=iw-1:h=ih-1:format=nv12:out_range=full[out]");
...
impl VulkanEncoder {
    pub fn alloc_frame(&self, drm_info: &DrmInfo) -> VulkanFrame {
        let drm_frame = DrmFrame::new(drm_info);
        VulkanFrame::new(drm_frame, self.hw_frame_ctx.clone())
    }

    #[hotpath::measure]
    pub fn encode(&mut self, hw_frame: &mut VulkanFrame, pts: i64) {
        unsafe {
            (*hw_frame.av_frame).pts = pts;
